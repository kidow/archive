Storybook [link](https://components.kidow.me/?path=/story/feedback--dialog)
...
```typescript title="containers/Dialog/index.tsx"
...
const Page = () => {
  ...
  return (
    <>
      <button onClick={() => ref.current?.showModal()}>Dialog Open</button>
