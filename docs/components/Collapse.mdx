...
Storybook [link](https://components.kidow.me/?path=/story/data-display--collapse)
        ...
        <div key={index} className="relative">
          <div
            className={classnames(
              'flex cursor-pointer items-center justify-between bg-gray-50 p-4 font-semibold',
              { 'border-b': item.open }
            )}
            onClick={() =>
              setState({
